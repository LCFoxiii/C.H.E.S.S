...
static constexpr int32_t piece_values[5] = {
    S(82, 94),   // pawn
    S(337, 281), // knight
    S(365, 297), // bishop
    S(477, 512), // rook
...
static constexpr int32_t PSQT_PAWN[48] = {
...
static constexpr int32_t PSQT_KNIGHT[64] = {
...
static constexpr int32_t PSQT_BISHOP[64] = {
...
static constexpr int32_t PSQT_ROOK[64] = {
...
static constexpr int32_t PSQT_QUEEN[64] = {
...
static constexpr int32_t PSQT_KING[64] = {
...
constexpr chess::Square flipped_relative_square(const chess::Color c, const chess::Square sq) {
    return chess::Square(sq.index() ^ (static_cast<int>(~c) * 56));
}
...
int32_t eval_all(const chess::Board& board, chess::Color color, int& phase) {
    ...
    while (our_pawns) {
        ...
        packed_score += PSQT_PAWN[flipped_relative_square(color, sq).index() - 8];
    }
    ...
    while (our_knights) {
        ...
        packed_score += PSQT_KNIGHT[flipped_relative_square(color, sq).index()];
    }
    ...
    while (our_bishops) {
        ...
        packed_score += PSQT_BISHOP[flipped_relative_square(color, sq).index()];
    }
    ...
    while (our_rooks) {
        ...
        packed_score += PSQT_ROOK[flipped_relative_square(color, sq).index()];
    }
    ...
    while (our_queens) {
        ...
        packed_score += PSQT_QUEEN[flipped_relative_square(color, sq).index()];
    }

    while (our_king) {
        chess::Square sq = our_king.pop();
        packed_score += PSQT_KING[flipped_relative_square(color, sq).index()];
    }
